<?php 
require '../class/db.php';
header('Content-Type: text/html; charset=UTF-8');
$id = Input::get('id');
$db = new DB();
$dbQuery = $db->query("produkt");
$iconQuery = $db->query("icon");
$photoQuery = $db->query("photos");

?>

<!DOCTYPE html>
<html>
<head>
	<title>NASLOV PROJEKTA :D</title>
	<link rel="shortcut icon" type="image/x-icon" href="../icons/icone.ico"/>
	<meta name="viewport" content="width=device-width,initial-scale=1, user-scalable=no" />
	<link rel="stylesheet" type="text/css" href="../css/index.css">
	<link rel="stylesheet" type="text/css" href="../css/mainindex.css">
	<link rel="stylesheet" type="text/css" href="../css/menu.css">
	<link rel="stylesheet" type="text/css" href="../css/dizajn.css">
	<link rel="stylesheet" type="text/css" href="../css/style.css">
	<script type="text/javascript" src="../js/app.js"></script>
	<script type="text/javascript" src="../js/jquery.js"></script>
	<script type="text/javascript" src="../js/slide.js"></script>
</head>
<body onload="init();">
	<?php require 'menu.php'; ?>
	<div id="loadingbar">
		<img src="../images/loading.gif">
	</div>
	<div id="mainWrapper">
		<?php 
		while($item = $dbQuery -> fetch()){
			if($item['id'] == $id){ ?>
				<div class="subwrapper subwrapper1">
					<?php 
					while($icon = $iconQuery -> fetch()){
						if($icon['idofpost'] == $item['id']){ ?>
							<div class="leftside">
								<img src="../admin/icons/<?php echo $icon['name']; ?>">
							</div>
						<?php }
					}
					?>
					<div class="leftside leftside1">
						<div id="paragraph">
							<p>
								<?php echo $item['bos']; ?>							
							</p>
						</div>
					</div>
				</div>
				<div class="subwrapper subwrapper2">
					<?php 
						$i = 1;
						while($photo = $photoQuery -> fetch()){
							if($photo['idofpost'] == $item['id']){ 
								$i++;
							}
						}
						$i--;
					?>
					<p id="numOfImages" style="display: none;"><?php echo $i; ?></p>
					<div id="slider">
					<?php 
					$photoQuery = $db->DESCquery("photos");
					while($photo = $photoQuery -> fetch()){
						if($photo['idofpost'] == $item['id']){ ?>
							
							<img class="sliderImages" id="<?php echo $i--; ?>" src="../admin/photos/<?php echo $photo['name']; ?>">		
						<?php }
					}
					?>

					<div class="arrows" >
						<div class="arrowRL">
							<img onclick="showPrevious();" src="../icons/arrowLeft.png">
						</div>
					</div>
					<div class="arrows arrows2">
						<div class="arrowRL">
							<img onclick="showNext();" src="../icons/arrowRight.png">
						</div>
					</div>
					</div>
					<?php 
					if(!empty($item['video'])){ ?>
						<div id="photoVideo">
							<div class="partofit">
								<p onclick="showFoto()">PHOTO</p>
							</div>
							<div class="partofit partofit2">
								<p onclick="showVideo();">VIDEO</p>
							</div>
						</div>
						<iframe id="ifr" src="<?php echo $item['video']; ?>" frameborder="0" allowfullscreen></iframe>
					<?php }
					?>
				</div>
			<?php }
		}
		?>
		<script type="text/javascript">
			function showVideo(){
				document.getElementById("slider").style.display = 'none';
				document.getElementById("ifr").style.display = 'block';
			}
			function showFoto(){
				document.getElementById("slider").style.display = 'block';
				document.getElementById("ifr").style.display = 'none';
			}
		</script>
		
		<?php require 'icons.php'; ?>
	</div>
</body>
</html>